...
def parseBU(UF, municipio, zona, secao):
    cod_mun = Data.getCodeMunicipio(UF, municipio)
    filename = "data\\{0}\\{1}\\o00406-{2}{3}{4}.bu".format(UF.upper(), municipio.upper(), str(cod_mun), zona, secao)
    
    #Se não encontrar arquivo *.BU busca arquivo *.BUSA
    if not os.path.exists(filename):
        filename = filename.replace(".bu", ".busa")
...
class objBU:
    ...
    def __init__(self, UF, Municipio, Zona, Secao):
        ...
        self.Secao = Secao
        self.dict = parseBU(UF, Municipio, Zona, Secao)

    def getQtdEleitoresAptos(self):
        return int(self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["qtdEleitoresAptos"])

    def getQtdComparecimento(self):
        return int(self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["resultadosVotacao"][0]["qtdComparecimento"])

    def getQtdCandidatos(self):
        return len(self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["resultadosVotacao"][0]["totaisVotosCargo"][0]["votosVotaveis"] - 2) #-2 para retirar branco e nulo

    def getVotosCandidato(self, index):
        return int.from_bytes(self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["resultadosVotacao"][0]["totaisVotosCargo"][0]["votosVotaveis"][index]["quantidadeVotos"], "big")

    def getCodigoCandidato(self, index):
        return int(self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["resultadosVotacao"][0]["totaisVotosCargo"][0]["votosVotaveis"][index]["identificacaoVotavel"]["codigo"])

    def getTipoVoto(self, index):
        return int.from_bytes(self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["resultadosVotacao"][0]["totaisVotosCargo"][0]["votosVotaveis"][index]["tipoVoto"], "big")

    def getVotosBrancos(self):
        return int.from_bytes(self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["resultadosVotacao"][0]["totaisVotosCargo"][0]["votosVotaveis"][5]["quantidadeVotos"], "big")

    def getVotosNulos(self):
        return int.from_bytes(self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["resultadosVotacao"][0]["totaisVotosCargo"][0]["votosVotaveis"][6]["quantidadeVotos"], "big")
    ...
    def getSetCandidatos(self):
        return self.dict["root"]["conteudo"]["entidadeBoletimUrna"]["resultadosVotacaoPorEleicao"][1]["resultadosVotacao"][0]["totaisVotosCargo"][0]["votosVotaveis"]
